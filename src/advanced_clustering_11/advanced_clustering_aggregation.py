...
from sklearn.cluster import AgglomerativeClustering, DBSCAN;  # class of clustering algorithms
...
single_cluster = AgglomerativeClustering(distance_threshold=0, 
                            n_clusters=None, linkage=link);  # instance of AgglomerativeClustering
...
# Generating clusters
single_cluster = AgglomerativeClustering(n_clusters=K, linkage=link); # instance of AgglomerativeClustering
single_cluster.fit(X);
pred_single = single_cluster.labels_;
db_single = davies_bouldin_score(X,pred_single); # goodness-of-fit
cm_single = contingency_matrix(y,pred_single); # comparing true label and cluster membership
...
# Generating clusters
complete_cluster = AgglomerativeClustering(n_clusters=K,linkage=link); # instance of AgglomerativeClustering
complete_cluster.fit(X);
pred_complete = complete_cluster.labels_;
db_complete = davies_bouldin_score(X,pred_complete); # goodness-of-fit
cm_complete = contingency_matrix(y,pred_complete);  # comparing true label and cluster membership
...
# Generating clusters
ward_cluster = AgglomerativeClustering(n_clusters=K,linkage=link); # instance of AgglomerativeClustering
ward_cluster.fit(X);
pred_ward = ward_cluster.labels_;
db_ward = davies_bouldin_score(X,pred_ward); # goodness-of-fit
cm_ward = contingency_matrix(y,pred_ward);  # comparing true label and cluster membership
...
cm_dbscan = contingency_matrix(y,pred_dbscan);  # comparing true label and cluster membership
