...
logreg = LogisticRegression(solver='liblinear');  # an instance of LogisticRegression class
...
proba_pred_sklearn = logreg.predict_proba(cancer.data)[:,1]; # aposterior probability of the positive (1) class 
# Prediction by numpy
...
plt.scatter(proba_pred_sklearn, proba_pred_numpy, s=50, c=cancer.target,
...
X_train, X_test, y_train, y_test = train_test_split(cancer.data,
        cancer.target, test_size = test_rate, random_state = 2024);
...
logreg1 = LogisticRegression(solver='liblinear'); # new instance of LogisticRegression class
...
plt.plot(bins,1/(score_std*np.sqrt(2*np.pi))*np.exp(-(bins-score_mean)**2/(2*score_std**2)),
