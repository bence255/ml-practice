...
from sklearn.metrics import confusion_matrix, ConfusionMatrixDisplay, roc_curve, auc, RocCurveDisplay; #  performance metrics
...
cm_naive_bayes_train = confusion_matrix(y_train, pred_naive_bayes_train); # train confusion matrix
cm_naive_bayes_test = confusion_matrix(y_test, pred_naive_bayes_test); # test confusion matrix 
...
disp = RocCurveDisplay(fpr=fpr_logreg, tpr=tpr_logreg, roc_auc=auc_logreg, estimator_name = 'Logistic regression');
disp.plot();
...
disp = RocCurveDisplay(fpr=fpr_naive_bayes, tpr=tpr_naive_bayes, roc_auc=auc_naive_bayes,estimator_name = 'naive Bayes');
disp.plot();
...
plt.plot(fpr_logreg, tpr_logreg, color='red',
         lw=lw, label='Logistic regression (AUC = %0.2f)' % auc_logreg);
plt.plot(fpr_naive_bayes, tpr_naive_bayes, color='blue',
         lw=lw, label='Naive Bayes (AUC = %0.2f)' % auc_naive_bayes);
