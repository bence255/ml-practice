...
X_train, X_test, y_train, y_test = train_test_split(
    digits.data, digits.target, test_size=0.3, random_state=2024
)
...
for i in range(k):
    custom_lines.append(
        Line2D(
            [0], [0], marker="o", color="w", markerfacecolor=colors[i], markersize=10
        )
    )
fig = plt.figure(4)
...
plt.ylabel("PC2")
plt.scatter(
    PC_train[:, 0], PC_train[:, 1], s=50, c=y_train, cmap=mcolors.ListedColormap(colors)
)
