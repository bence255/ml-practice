...
# Reading the dataset
url = (
    "https://arato.inf.unideb.hu/ispany.marton/DataMining/Practice/Datasets/bodyfat.csv"
)
raw_data = urlopen(url)
attribute_names = np.loadtxt(
    raw_data, max_rows=1, delimiter=",", dtype=np.str
)  # reading the first row with attribute names
...
df = pd.DataFrame(
    data=data, columns=attribute_names
