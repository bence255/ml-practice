...
print(
    diabetes.feature_names[feature], "[", record, "]:", diabetes.data[record, feature]
)
del user_input

# Partitioning into training and test sets
X_train, X_test, y_train, y_test = train_test_split(
    diabetes.data, diabetes.target, test_size=0.2, random_state=2024
)
...
pred_numpy = intercept * np.ones((n)) + np.dot(
    diabetes.data, weights
)  # prediction by numpy
...
R2_numpy = 1 - np.dot(error, error) / np.dot(
    centered_target, centered_target
)  # computing R-square by numpy
