...
for i in range(rep):
    ...
    x_sample = x[index];
    y_sample = y[index];
    X_sample = x_sample.reshape(1, -1).T;  # transforming 1d array into 2D array (necessary for fit method)
...
print(f'Mean slope:{w1hat_mean:6.4f} (True slope:{w1}) with standard deviation {w1hat_std:6.4f}');
print(f'Mean intercept:{w0hat_mean:6.4f} (True intercept:{w0}) with standard deviation {w0hat_std:6.4f}');
print(f'Mean of R-square for goodness of fit:{R2_mean:6.4f} (standard deviation: {R2_std:6.4f})');
...
plt.text(w1-2.5*w1hat_std, 10, f'$\\mu={w1hat_mean:4.3f},\\ \\sigma={w1hat_std:4.3f}$');
...
plt.text(w0-2.5*w1hat_std, 10, f'$\\mu={w0hat_mean:4.3f},\\ \\sigma={w0hat_std:4.3f}$');
...
for i in range(rep):
    ...
    x_test = x[index];
    y_test = y[index];
    X_test = x_test.reshape(1, -1).T;   # transforming 1d array into 2D array (necessary for fit method)
...
# Printing the results
print(f'Mean of test R-squares:{R2_mean:6.4f} (standard deviation: {R2_std:6.4f}), training R-square: {R2_train:6.4f}');
...
# Printing the results
print(f'Mean of R-square in crossvalidation:{R2_mean:6.4f} (standard deviation: {R2_std:6.4f})');
