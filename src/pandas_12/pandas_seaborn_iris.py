...
pd.plotting.andrews_curves(
    iris.frame, class_column="target", color=["blue", "green", "red"]
)
plt.show()
# Parallel axis
plt.figure(2)
...
sns.relplot(
    data=iris.frame,
    x=iris.feature_names[x_axis],
    y=iris.feature_names[y_axis],
    hue="target",
    palette=colors,
)
...
plt.figure(8)
sns.histplot(
    data=iris.frame, x=iris.feature_names[x_axis], hue="target", palette=colors
)
plt.show()
