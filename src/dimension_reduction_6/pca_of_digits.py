...
Corrected on Tue Nov 19 18:09:15 2024

Task: Principal Component Analysis (PCA) of digits dataset
Results: 2D plots
...
Functions: int16, load_digits, scatter, bar, ListedColormap
...
from sklearn.datasets import load_digits; # digit dataset loader
...
X_train, X_test, y_train, y_test = train_test_split(digits.data, 
             digits.target, test_size=0.3, random_state=2024);

# Full PCA on training set
pca = PCA();   # an instance of PCA
pca.fit(X_train);  # fitting PCA for training data
...
PC_train = pca.transform(X_train); # transforming training data to the PC space
custom_lines = [];
colors = list(mcolors.TABLEAU_COLORS.keys());  # 
for i in range(k):
    custom_lines.append(Line2D([0], [0], marker='o', color='w', markerfacecolor=colors[i], markersize=10));
fig = plt.figure(4);
plt.title('Scatterplot for training digits dataset');
plt.xlabel('PC1');
plt.ylabel('PC2');
plt.scatter(PC_train[:,0],PC_train[:,1],s=50,c=y_train,cmap = mcolors.ListedColormap(colors));
plt.legend(custom_lines, digits.target_names, loc='lower right');
plt.show();

# Visualizing the test set in 2D PC space
PC_test = pca.transform(X_test); # transforming test data to the PC space
fig = plt.figure(5);
