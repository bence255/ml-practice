...
d = {'input':x,'latent':z,'probability':p,'target':y};  # dictionary for generated data
...
d_logistic = {'x':np.arange(-5,5,res),'f(x)':sp.special.expit(np.arange(-5,5,res))};
...
ax = df.plot.scatter(x='input', y='probability', color="black");  # 1st plot for latent probabilities
ax2 = df.plot.scatter(x='input', y='target', c=y, cmap=mcolors.ListedColormap(colors), ax=ax);  # 2nd plot for datapoints
...
proba_pred_logreg = logreg.predict_proba(df_input);  # posterior distribution for the target
...
ax = df.plot.scatter(x='probability', y='posterior', color='blue', label='probabilities');  # probabilities plot
line = pd.DataFrame(data=[[0,0], [1,1]]);   # the 2 endpoints of diagonal line
line.plot.line(x=0, y=1, color='black', label='diagonal',ax=ax);  # drawing the diagonal line
